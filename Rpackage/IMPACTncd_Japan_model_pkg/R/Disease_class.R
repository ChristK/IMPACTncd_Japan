## IMPACTncdJapan is an implementation of the IMPACTncd framework, developed by
...
##
## IMPACTncdJapan is free software; you can redistribute it and/or modify it
  ...
  R6::R6Class(
    ...
    public = list(
      ...
      gen_parf_files = function(design_ = design, diseases_ = diseases,
        ...
        if (sum(dim(private$incd_indx)) > 0) {
          ...
          tt <- self$get_incd(yrs, 0L, design_ = design_)
          nam <- "p0"

          parf_dt <- clone_dt(parf_dt, length(yrs)) # works even if length(yrs) == 1
          parf_dt[, `:=` (
            year = .id - 1L + design_$sim_prm$init_year_long,
            .id = NULL
          )]
        ...
        if (sum(dim(private$ftlt_indx)) > 0) {
          ...
          tt <- self$get_ftlt(yrs, 0L, design_ = design_)
      ...
      set_mrtl_prb = function(sp, design_ = design) {
        ...
        if (is.numeric(self$meta$mortality$type)) {
          ...
          if ((!design_$sim_prm$apply_RR_to_mrtl2 &&
            ...
            if (FALSE) { # design_$sim_prm$init_ftlt_calibration
            # Calibrate estimated fatality prbl to init year incidence
            tbl <- self$get_ftlt(design_$sim_prm$init_year, mc_ = sp$mc_aggr, design_ = design_
            )[between(age, design_$sim_prm$ageL,
                      design_$sim_prm$ageH)]
            ...
            if (self$name == "nonmodelled") {
              tbl <- sp$pop[year == design_$sim_prm$init_year_long,
            ...
            if (FALSE) { # design_$sim_prm$mortality_calibration
              ...
              sp$pop[year >= design_$sim_prm$init_year_long,
                     clbfctr := clbfctr * mrtl_clbr * clbintrc *
                       (clbtrend^(year - design_$sim_prm$init_year_long))]
            }
            # End of calibration
    ...
    private = list(
        ...
        function(mc_, ff, design_, diseases_) {
          ...
          if (any(xps %in% xps_dep$xpscol)) {
            if (xps_dep[xps[1:2], ][!is.na(ds), any(duplicated(ds))]) {
              stop("Smoking & Smoking_number cannot coexist as risk factors for a disease.")
            }
      ...
      ff[Smoking_number_grp == 7L, Smoking_number_curr_xps := 40L]
	    ff[Smoking_number_grp == 8L, Smoking_number_curr_xps := sample(c(50L, 60L, 80L), .N, TRUE, prob = c(0.4, 0.45, 0.15))]
      ...
      if (any(xps %in% xps_dep$xpscol)) {
        ...
        tbl[, sex := factor(sex, 0:1, c("men", "women")), ]
			  ...
			  ff[, PA_3cat := fifelse(PA_days_curr_xps %in% as.character(0:1), 1L,
			   			fifelse(PA_days_curr_xps %in% as.character(2:4), 2L,
      ...
      if (any(xps %in% xps_dep$xpscol)) {
        ...
        tbl <-
          read_fst("./inputs/exposure_distributions/Table_HbA1c.fst",
                   as.data.table = TRUE)
        setnames(tbl, c("Age", "Sex", "Year", "BMI", "Med_DM"),
      ...
      if (any(xps %in% xps_dep$xpscol)) {
        ...
        tbl <-
          read_fst("./inputs/exposure_distributions/Table_LDLc.fst",
                   as.data.table = TRUE)
        setnames(tbl, c("Age", "Sex", "Year", "BMI", "Med_HL"),
...
fit_beta = # NOT VECTORISED
  ...
  # Fix given path so relative to current work directory.
	...
	FixPathSoRelativeWorkDir = function(sGivenPath) {
		#return(sGivenPath)
		sProjectTopDir <- "IMPACTncd_Japan/"

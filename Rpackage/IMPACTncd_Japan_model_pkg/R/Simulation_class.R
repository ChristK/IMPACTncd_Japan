  ...
  R6::R6Class(
    ...
    public = list(
      ...
      calibrate_incd_ftlt = function(mc, replace = FALSE) {
        ...
        for (age_ in age_start:self$design$sim_prm$ageH) {
          ...
          unclbr <- open_dataset(file.path(self$design$sim_prm$output_dir, "summaries", "incd_scaled_up")) %>%
            filter(age == age_) %>%
            select("year", "age", "sex", "mc", "popsize", "chd_incd", "stroke_incd") %>%
            collect()
          setDT(unclbr)

          unclbr <- unclbr[, .(chd_incd = chd_incd / popsize, stroke_incd = stroke_incd / popsize), keyby = .(age, sex, year, mc)
            ][, .(chd_incd = memedian(chd_incd), stroke_incd = memedian(stroke_incd)), keyby = .(age, sex, year)]
          ...
          prvl <- open_dataset(file.path(self$design$sim_prm$output_dir, "summaries", "prvl_scaled_up")) %>%
            filter(age == age_) %>%
            select("year", "age", "sex", "mc", "popsize", "chd_prvl", "stroke_prvl") %>%
            collect()
          setDT(prvl)
          ...
          mrtl <- open_dataset(file.path(self$design$sim_prm$output_dir, "summaries", "mrtl_scaled_up")) %>%
            filter(age == age_ - 1L) %>%
            select("year", "age", "sex", "mc", "popsize", "chd_deaths", "stroke_deaths", "nonmodelled_deaths") %>%
            collect()
          setDT(mrtl)
      ...
      export_summaries = function(multicore = TRUE,
        ...
        if (file.exists(file_pth) && length(list.files(file_pth)) > 0) {
          ...
          duckdb::duckdb_register_arrow(con2, "tbl", open_dataset(file_pth, format = "parquet"))
        ...
        if (multicore) {
          ...
          if (.Platform$OS.type == "windows") {
            ...
            parLapplyLB(
              ...
              X = seq_along(mc_set),
              fun = function(i) {
                private$export_summaries_hlpr(con, mcaggr = i, type = type, single_year_of_age = single_year_of_age)
                NULL
              }
            )
          } else {
      ...
      export_tables = function(baseline_year_for_change_outputs = 2019L,
                               prbl = c(0.5, 0.025, 0.975, 0.1, 0.9)) {
         private$export_main_tables(prbl, baseline_year_for_change_outputs, private$output_dir())
         private$export_all_cause_mrtl_tables(prbl, private$output_dir("summaries"), private$output_dir("tables"))
         private$export_disease_characteristics_tables(prbl, private$output_dir("summaries"), private$output_dir("tables"))
         private$export_xps_tables(prbl, private$output_dir(), private$output_dir("tables"))
      ...
      validate = function() {
        ...
        # MRTL
        mdd <- open_dataset(file.path(self$design$sim_prm$output_dir, "summaries", "dis_mrtl_scaled_up")) %>%
          filter(scenario == "sc0") %>%
          collect()
        setDT(mdd)
        mdd[, `:=`(
        ...
        # INCD
        mdd <- open_dataset(file.path(self$design$sim_prm$output_dir, "summaries", "incd_scaled_up")) %>%
          filter(scenario == "sc0") %>%
          select("mc", "scenario", "year", "agegrp", "sex", "popsize", "chd_incd", "stroke_incd") %>%
          collect()
        setDT(mdd)
        mdd[, `:=`(
        ...
        # PRVL
        mdd <- open_dataset(file.path(self$design$sim_prm$output_dir, "summaries", "prvl_scaled_up")) %>%
          filter(scenario == "sc0") %>%
          select("mc", "scenario", "year", "agegrp", "sex", "popsize", "chd_prvl", "stroke_prvl") %>%
          collect()
        setDT(mdd)
    ...
    private = list(
      ...
      export_xps = function(sp, scenario_nam) {
        ...
        fileformat <- "parquet"
        fnam <- private$output_dir(paste0(
          "xps/xps20/", "scenario=", scenario_nam, "/", "mc=", sp$mc_aggr, "/", sp$mc, "_xps20.", fileformat
        ))
        # NOTE parquet format about 30 times smaller but about 50% slower in writting to disk
        ...
        fnam <- private$output_dir(paste0(
          "xps/xps5/", "scenario=", scenario_nam, "/", "mc=", sp$mc_aggr, "/", sp$mc, "_xps_esp.", fileformat
        ))
        # NOTE parquet format about 30 times smaller but about 50% slower in writting to disk
      ...
      export_summaries_hlpr = function(duckdb_con, mcaggr, type = c(
                                         "le", "hle", "dis_char",
        ...
        if ("le" %in% type) private$export_le_summaries(duckdb_con, mcaggr, strata_noagegrp, ext)
        if ("hle" %in% type) private$export_hle_summaries(duckdb_con, mcaggr, strata_noagegrp, ext)
        if ("dis_char" %in% type) private$export_dis_char_summaries(duckdb_con, mcaggr, strata_noagegrp, ext)
        if ("prvl" %in% type) private$export_prvl_summaries(duckdb_con, mcaggr, strata, ext)
        if ("incd" %in% type) private$export_incd_summaries(duckdb_con, mcaggr, strata, ext)
        if ("mrtl" %in% type) private$export_mrtl_summaries(duckdb_con, mcaggr, strata, ext)
        if ("dis_mrtl" %in% type) private$export_dis_mrtl_summaries(duckdb_con, mcaggr, strata, ext)
        if ("all_cause_mrtl_by_dis" %in% type) private$export_all_cause_mrtl_by_dis_summaries(duckdb_con, mcaggr, strata, ext)
        if ("cms" %in% type) private$export_cms_summaries(duckdb_con, mcaggr, strata, strata_age, ext)
        if ("qalys" %in% type) private$export_qaly_summaries(duckdb_con, mcaggr, strata, ext)
        if ("costs" %in% type) private$export_costs_summaries(duckdb_con, mcaggr, strata, ext)
      ...
      calc_QALYs = function(duckdb_con, mcaggr, input_table_name, output_view_name, include_non_significant = FALSE) {
        eq5d5l_expr <- "
          0.989
          + CASE agegrp
        ...
        if (!include_non_significant) { # Original logic: if (!include_non_significant == TRUE)
          eq5d5l_expr <- paste0(
            eq5d5l_expr,
        ...
        create_view_sql <- sprintf("
          ...
          FROM %s
          WHERE mc = %d;
        ", output_view_name, eq5d5l_expr, hui3_expr, input_table_name, mcaggr)
...
calc_costs = function(duckdb_con, mcaggr, input_table_name, output_view_name) {
  ...
  return(invisible(NULL))
      ...
      create_new_folder = function(sDirPathName, bReport = self$design$sim_prm$logs) {
      ...
      export_le_summaries = function(duckdb_con, mcaggr, strata_noagegrp, ext) {
        lapply(paste0(rep(c("le", "le60"), each = 2), "_", c("scaled_up", "esp")), function(subdir) {
          private$create_new_folder(private$output_dir(paste0("summaries/", subdir)))
        })
        ...
        le_configs <- list(
          list(prefix = "le", age_filter = "", weight_col = "wt", suffix = "_scaled_up"),
          list(prefix = "le", age_filter = "", weight_col = "wt_esp", suffix = "_esp"),
          list(prefix = "le60", age_filter = "AND age > 60", weight_col = "wt", suffix = "_scaled_up"),
          list(prefix = "le60", age_filter = "AND age > 60", weight_col = "wt_esp", suffix = "_esp")
        )

        for (config in le_configs) {
          # Skip LE60 calculation if age range doesn't cover 60
          if (startsWith(config$prefix, "le60") && !(self$design$sim_prm$ageL < 60L && self$design$sim_prm$ageH > 60L)) {
            next
          }
          ...
          query <- sprintf(
            ...
            GROUP BY %s
            ORDER BY %s",
            group_by_cols, config$weight_col, config$weight_col, config$weight_col, # Use weight_col 3 times
            mcaggr, config$age_filter, group_by_cols, group_by_cols
          )

          output_path <- private$output_dir(
            paste0("summaries/", config$prefix, config$suffix, "/", mcaggr, "_", config$prefix, config$suffix, ".", ext)
          )
      ...
      export_hle_summaries = function(duckdb_con, mcaggr, strata_noagegrp, ext) {
        ...
        lapply(paste0(rep(c("hle_1st_cond", "hle_cmsmm1.5"), each = 2), "_", c("scaled_up", "esp")), function(subdir) {
          private$create_new_folder(private$output_dir(paste0("summaries/", subdir)))
        })
        ...
        hle_configs <- list(
          list(prefix = "hle_1st_cond", condition = "cms_count = 1", weight_col = "wt", suffix = "_scaled_up"),
          list(prefix = "hle_1st_cond", condition = "cms_count = 1", weight_col = "wt_esp", suffix = "_esp"),
          list(prefix = "hle_cmsmm1.5", condition = '"cmsmm1.5_prvl" = 1', weight_col = "wt", suffix = "_scaled_up"),
          list(prefix = "hle_cmsmm1.5", condition = '"cmsmm1.5_prvl" = 1', weight_col = "wt_esp", suffix = "_esp")
        )
        ...
        for (config in hle_configs) {
          query <- sprintf(
            "SELECT %s, SUM(%s) AS popsize, SUM(age * %s) / SUM(%s) AS HLE
               ...
               GROUP BY %s
               ORDER BY %s",
            group_by_cols, config$weight_col, config$weight_col, config$weight_col,
            mcaggr, config$condition, group_by_cols, group_by_cols
          )
      ...
      export_dis_char_summaries = function(duckdb_con, mcaggr, strata_noagegrp, ext) {
        lapply(paste0(rep(c("dis_characteristics"), each = 2), "_", c("scaled_up", "esp")), function(subdir) {
          private$create_new_folder(private$output_dir(paste0("summaries/", subdir)))
        })
        ...
        select_and_group_by_cols_sql <- paste0("t.", strata_noagegrp, collapse = ", ")
        ...
        if (length(nm) > 0) {
          ...
          union_queries <- lapply(nm, function(disease_col) {
            ...
            sprintf(
              ...
              lc_table_name, mcaggr, quoted_disease_col,
              lc_table_name, mcaggr,
              select_and_group_by_cols_sql,
              disease_name,
              quoted_disease_col, quoted_disease_col,
              quoted_disease_col,
              lc_table_name, mcaggr, quoted_disease_col,
              select_and_group_by_cols_sql,
          ...
          pivot_query <- sprintf("
           ...
           ", full_union_query, paste0("'", gsub("_prvl$", "", nm), "'", collapse = ", "))
          ...
          dbExecute(duckdb_con, sprintf(
            "COPY (%s) TO '%s' (FORMAT PARQUET);",
            wrapped_sql_esp, output_path_esp
          ))
          NULL
        } # length(nm) > 0
      ...
      export_prvl_summaries = function(duckdb_con, mcaggr, strata, ext) {
        ...
        # disease counter and score can be reduced.
        # Ideally only the first reach to the threshold should be counted
        lapply(paste0(rep(c("prvl"), each = 2), "_", c("scaled_up", "esp")), function(subdir) {
          private$create_new_folder(private$output_dir(paste0("summaries/", subdir)))
        })
        ...
        sum_cases_cols <- paste(sprintf('SUM(CASE WHEN "%s" > 0 THEN wt ELSE 0 END) AS "%s"', nm_prvl, nm_prvl), collapse = ", ")
      ...
      export_incd_summaries = function(duckdb_con, mcaggr, strata, ext) {
        ...
        # disease counter and score can be reduced.
        # Ideally only the first reach to the threshold should be counted
        lapply(paste0(rep(c("incd"), each = 2), "_", c("scaled_up", "esp")), function(subdir) {
          private$create_new_folder(private$output_dir(paste0("summaries/", subdir)))
        })
        ...
        # Construct the SQL query dynamically
        select_cols <- paste(strata, collapse = ", ")
        sum_cases_cols <- paste(sprintf('SUM(CASE WHEN "%s" = 1 THEN wt ELSE 0 END) AS "%s"', nm_prvl, gsub("_prvl$", "_incd", nm_prvl)), collapse = ", ")
      ...
      export_mrtl_summaries = function(duckdb_con, mcaggr, strata, ext) {
        lapply(paste0(rep(c("mrtl"), each = 2), "_", c("scaled_up", "esp")), function(subdir) {
          private$create_new_folder(private$output_dir(paste0("summaries/", subdir)))
        })
      ...
      export_dis_mrtl_summaries = function(duckdb_con, mcaggr, strata, ext) {
        lapply(paste0(rep(c("dis_mrtl"), each = 2), "_", c("scaled_up", "esp")), function(subdir) {
          private$create_new_folder(private$output_dir(paste0("summaries/", subdir)))
        })
        ...
        pivoted_death_col_names_all <- paste0(names(private$death_codes), "_deaths")
        quoted_pivoted_death_col_names_all <- paste0('"', pivoted_death_col_names_all, '"')


        # Correctly format the IN clause with quoted aliases for PIVOT (includes 'alive_deaths')
        death_codes_pivot_sql <- paste0(
          "'", private$death_codes, "' AS ", # Use single quotes for values, double for aliases
        ...
        pivoted_death_col_names_final <- setdiff(pivoted_death_col_names_all, "alive_deaths")
        quoted_pivoted_death_col_names_final <- paste0('"', pivoted_death_col_names_final, '"')
        ...
        coalesce_select_sql_final <- paste0(
          "COALESCE(t.", quoted_pivoted_death_col_names_final, ", 0) AS ",
        ...
        death_cols_sum_sql <- paste0("COALESCE(t.", quoted_pivoted_death_col_names_all, ", 0)", collapse = " + ")
        ...
        select_strata_sql, coalesce_select_sql_final, death_cols_sum_sql, sql_query, strata_cols_sql) # Use quoted strata_cols_sql for ORDER BY

        output_path <- private$output_dir(
          paste0("summaries/dis_mrtl_scaled_up/", mcaggr, "_dis_mrtl_scale_up.", ext)
        )

        dbExecute(duckdb_con, sprintf(
          "COPY (%s) TO '%s' (FORMAT PARQUET);",
          sql_query_final, output_path
        ))
      ...
      export_all_cause_mrtl_by_dis_summaries = function(duckdb_con, mcaggr, strata, ext) {
        lapply(paste0(rep(c("all_cause_mrtl_by_dis"), each = 2), "_", c("scaled_up", "esp")), function(subdir) {
          private$create_new_folder(private$output_dir(paste0("summaries/", subdir)))
        })
        ...
        case_statements <- sapply(disease_names, function(dis) {
          ...
          sprintf('SUM(CASE WHEN %s > 0 THEN wt ELSE 0 END) AS "cases_%s"', prvl_col, dis)
        })

        death_statements <- sapply(disease_names, function(dis) {
          prvl_col <- paste0('"', dis, '_prvl"')
          sprintf('SUM(CASE WHEN %s > 0 AND all_cause_mrtl > 0 THEN wt ELSE 0 END) AS "deaths_%s"', prvl_col, dis)
        })

        # Combine all select parts
        select_parts_sql <- paste(c(strata_cols_sql, case_statements, death_statements), collapse = ",\n  ")
        ...
        GROUP BY %s
        ORDER BY %s
        ", select_parts_sql, lc_table_name, mcaggr, strata_cols_sql, strata_cols_sql)

        output_path <- private$output_dir(
          paste0("summaries/all_cause_mrtl_by_dis_scaled_up/", mcaggr, "_all_cause_mrtl_by_dis_scale_up.", ext)
        )

        dbExecute(duckdb_con, sprintf(
          "COPY (%s) TO '%s' (FORMAT PARQUET);",
          sql_query_dis_char, output_path
        ))

        # esp
        sql_query_esp <- gsub("wt", "wt_esp", sql_query_dis_char)
        output_path_esp <- private$output_dir(
          paste0("summaries/all_cause_mrtl_by_dis_esp/", mcaggr, "_all_cause_mrtl_by_dis_esp.", ext)
        )
        dbExecute(duckdb_con, sprintf(
          "COPY (%s) TO '%s' (FORMAT PARQUET);",
          sql_query_esp, output_path_esp
      ...
      export_cms_summaries = function(duckdb_con, mcaggr, strata, strata_age, ext) {
        lapply(paste0(rep(c("cms_score", "cms_score_by_age", "cms_count"), each = 2), "_", c("scaled_up", "esp")), function(subdir) {
        private$create_new_folder(private$output_dir(paste0("summaries/", subdir)))
        })
        ...
        cms_configs <- list(
          list(metric = "cms_score", group_cols_sql = strata_sql, group_cols_r = strata, weight_col = "wt", suffix = "_scaled_up", file_group_suffix = ""),
          list(metric = "cms_score", group_cols_sql = strata_age_sql, group_cols_r = strata_age, weight_col = "wt", suffix = "_by_age_scaled_up", file_group_suffix = "_by_age"),
          list(metric = "cms_score", group_cols_sql = strata_sql, group_cols_r = strata, weight_col = "wt_esp", suffix = "_esp", file_group_suffix = ""),
          list(metric = "cms_score", group_cols_sql = strata_age_sql, group_cols_r = strata_age, weight_col = "wt_esp", suffix = "_by_age_esp", file_group_suffix = "_by_age"),
          list(metric = "cms_count", group_cols_sql = strata_sql, group_cols_r = strata, weight_col = "wt", suffix = "_scaled_up", file_group_suffix = ""),
          list(metric = "cms_count", group_cols_sql = strata_sql, group_cols_r = strata, weight_col = "wt_esp", suffix = "_esp", file_group_suffix = "")
        )
        ...
        for (config in cms_configs) {
          query <- sprintf(
            ...
            config$weight_col,
            config$metric, config$weight_col, config$weight_col, config$metric,
            lc_table_name,
            mcaggr,
            config$group_cols_sql,
            config$group_cols_sql
          ...
          output_path <- private$output_dir(
            paste0("summaries/", config$metric, config$file_group_suffix, ifelse(config$weight_col == "wt_esp", "_esp", "_scaled_up"),
                   "/", mcaggr, "_", config$metric, config$file_group_suffix, ifelse(config$weight_col == "wt_esp", "_esp", "_scaled_up"), ".", ext)
          )
      ...
      export_qaly_summaries = function(duckdb_con, mcaggr, strata, ext) {
        lapply(paste0(rep(c("qalys"), each = 2), "_", c("scaled_up", "esp")), function(subdir_suffix) {
        ...
        dbExecute(duckdb_con, sprintf("DROP VIEW IF EXISTS %s;", qaly_view_name))

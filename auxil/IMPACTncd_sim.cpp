...
struct disease_epi
{
  ...
  std::unordered_map<int, bool>  flag; // to hold flag per pid
  int cure;
  int death_code;
};
...
disease_meta get_disease_meta(const List diseaseFields, DataFrame dtSynthPop, const List l)
{
  ...
  if (diseaseFields.containsElementNamed("incidence"))
  {
    ...
    for (int num : pid)
    {
      out.incd.flag[num] = false; // set to false for all pids. If true incident occured during lifecourse
    }
  ...
  if (diseaseFields.containsElementNamed("diagnosis"))
  {
    ...
    for (int num : pid)
    {
      out.dgns.flag[num] = false; // set to false for all pids. Currently unused
    }
  }
  ...
  if (diseaseFields.containsElementNamed("mortality"))
  {
    ...
    for (int num : pid)
    {
      out.mrtl.flag[num] = false; // set to false for all pids. If true denotes cure
    }
...
inline void DiseaseIncidenceType2(vector<disease_meta> &dsmeta, int j, int i, double rn1, simul_meta &meta)
{
    ...
    {
        if (!dsmeta[j].incd.flag[meta.pid[i]] && VECT_ELEM(dsmeta[j].incd.prvl, i - 1) == 0 &&
            rn1 <= VECT_ELEM(dsmeta[j].incd.prbl1, i))
        {
            VECT_ELEM(dsmeta[j].incd.prvl, i) = 1;
            dsmeta[j].incd.flag[meta.pid[i]] = true; // Set flag
        }
    ...
    if (dsmeta[j].incd.type == "Universal" || VECT_ELEM(dsmeta[j].incd.prvl, i) > 0) // enter branch only for prevalent cases or Universal incidence
    {
        ...
        else if (dsmeta[j].mrtl.type == "Type2")
        {
            if (VECT_ELEM(dsmeta[j].incd.prvl, i) < dsmeta[j].mrtl.cure) // Valid since not universal incidence
            {
              ...
              dsmeta[j].mrtl.flag[meta.pid[i]] = true; // if alive cure after defined period
...
void simcpp(DataFrame dt, const List l, const int mc)
{
  ...
  for (int j = 0; j < dn; ++j) dsmeta[j] = get_disease_meta(diseases[j], dt, l);
  ...
  for (int i = 0; i < n; ++i) // loop over dt rows (and resolve all diseases for each row before you move on)
  {
    ...
    if ((i==0 || meta.dead[i-1]==SIMULANT_ALIVE) && meta.year[i] >= meta.init_year && meta.age[i] >= meta.age_low)
    {
      ...
      for (int j = 0; j < dn; ++j) // loop over diseases
      {
        ...
        if (dsmeta[j].mrtl.flag[meta.pid[i]])
        {
          ...
          dsmeta[j].mrtl.flag[meta.pid[i]] = false;
        }

        EvalDiseaseIncidence(dsmeta, i, j, rn1, mltp, meta);
        EvalDiagnosis(dsmeta,rn1, j, i, meta);
        EvalMortality(dsmeta, tempdead, i, j, rn1, mltp, meta);
